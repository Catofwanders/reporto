---
description: Triage both inboxes via the Chrome extension into a styled HTML report opened in the browser, and write the email + calendar reports the dashboard reads. Only actionable/personal items — pings on my PRs and tickets, personal mail with attachments, trainings or compliance needing action. No CI/status noise. Constants and filter rules in ~/.claude/docs/email-helper.md.
---

Triage both inboxes into one HTML report, open it in browser.

**First read `~/.claude/docs/email-helper.md`** — URLs, reading flow, filter rules,
report format. Follow it exactly; don't rediscover.
...
4. Copy `~/.claude/docs/email-report-template.html` to scratchpad `email-report.html`,
   fill slots ({{DATE}}, accounts, counts, items, filtered-out note). Keep template CSS untouched.
5. Write `email-<date>.json` and `calendar-<date>.json` into the dashboard's
   `public/reports/`, and bump `index.json` — see the report contract in the helper.
6. `open <scratchpad>/email-report.html` and give a 3-line terminal summary
   (most urgent first). Close the browser tab I used for reading mail.
